...
capacity=1000000000
bottleneckprob =0#1 / 100000
mutationrate=0.003*0.5*0.75
...
class allele:
    ...
    def update(self, population,availablebens, availablerevs,mutantstoadd,norm): #Increase population size and add mutants
        ...
        for i in range(len(mutantclasses)):
            ...
            #Adding in the recombinants here
            newmuts[3]=newmuts[3]+numpy.random.poisson(self.size*self.genotype[i]*2.5*10**-7)
            #print(newmuts)
    ...
    def get_size(self): #Return size
        return self.size
    def set_size(self, value):  # Return size
        self.size=value
    def get_reversions(self): #Return number of reversions multiplied by population (reversion mass?)
        #Modify here to just count recombinants to figure out dS
        tosum=self.genotype[4]
...
if __name__ == '__main__':
        ...
        for u in range(numberofruns):
            ...
            for k in range(numbergens):
                ...
                for x in currentclasses:
                    ...
                    if spec_size>=1 and minimum_load_contend<minimum_load:
                        minimum_load=minimum_load_contend
                        least_loaded_spec=x
                ...
                if False:#numpy.random.uniform(0,1)<((1/(8400))): #edited
                    numbertoadd=numpy.random.poisson(1,1)
                    availablebens=availablebens+numbertoadd
    ...
    numpy.savetxt('ds_recombination.txt', save1)
    numpy.savetxt('ds_syn.txt', save2)
