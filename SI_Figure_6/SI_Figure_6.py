...
Ns_sim=numpy.loadtxt("ds_recombination.txt")
dN_dS_sim=numpy.zeros(numpoints)
sim_fitness=numpy.ones(numpoints)

Ns_exclude_adap=numpy.loadtxt("mutation1.txt")
dN_dS_exclude_adap=numpy.zeros(numpoints)
exclude_adap_fitness=numpy.ones(numpoints)

S_bonus=numpy.loadtxt("ds_syn.txt")
dN_dS_sim_bonus_ds=numpy.zeros(numpoints)
...
for n in range(0,numpoints):
    ...
    dN_dS_sim_bonus_ds[n] = normalization_constant * (
    (((neutral_nonsynonymous_muts[n] + 2 * Ns_sim[n]) / (synonymous_muts[n]+(S_bonus[n])))))
...
ax3.plot(S_rate_per_codon * 2 * T_generations, dN_dS_exclude_adap, linewidth=3, label='simulation $N_e=10^9$')
ax3.plot(S_rate_per_codon * 2 * T_generations, dN_dS_sim, linewidth=3, label='simulation (recombination/no $d_S$ brought in)', color='tab:blue', linestyle='--')
ax3.plot(S_rate_per_codon * 2 * T_generations+S_bonus/core_genome_codons, dN_dS_sim_bonus_ds, linewidth=3, label='simulation (recombination/$d_S$ brought in)', color='tab:red', linestyle='--',zorder=0)
...
ax3.set_ylim([0.05,5])
ax3.legend(fontsize='9',ncol=2,frameon=False)
...
ax2.plot(T_generations, exclude_adap_fitness, linewidth=3, label='simulation $N_e=10^9$')

ax2.plot(T_generations, sim_fitness, linewidth=3, label='simulation (including recombination)', color='tab:blue', linestyle='--')
