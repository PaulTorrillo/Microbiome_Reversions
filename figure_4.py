...
xdata=numpy.loadtxt("observed_gene_dNdS.txt")
ydata=numpy.loadtxt("actual_gene_dNdS.txt")
x=numpy.linspace(0,20000000,21)
x=x[1:]
ax1.plot(x,xdata,label='observed',linewidth='3')
ax1.plot(x,ydata,label='actual',linewidth='3',color='tab:blue',linestyle=':')
ax1.legend(fontsize=10,frameon=False)
ax1.set_xlabel('bacterial generations', size=14)
ax1.set_ylabel('gene dN/dS', size=14)
ax1.set_yscale('log')
...
ax3.plot(S_rate_per_codon * 2 * T_generations, dN_dS_sim_act, linewidth=3, label='simulation (actual)', color='tab:blue', linestyle=':')

ax3.plot(S_rate_per_codon*2*T_generations,numpy.ones_like(dN_dS_theory),linewidth=3,label='dN/dS=1',linestyle='--',color='k')
